...
module Tanker
  ...
  class << self
    ...
    def search_results(models, query, options = {})
      ...
      #this query works..but it makes those really long queries ... I THINK
      ## THIS WORKS --> query = "(#{search_on_fields}:(#{query.to_s})) __type:(#{models.map(&:name).map {|name| "\"#{name.split('::').join(' ')}\"" }.join(' OR ')})"
